...
class Kele
  ...
  def update_submission(assignment_branch, assignment_commit_link, checkpoint_id, checkpoint_submission_id, comment)
    self.get_me
    body = {
      "assignment_branch": assignment_branch,
      "assignment_commit_link": assignment_commit_link,
    ...
    response = self.class.put("/checkpoint_submissions/#{checkpoint_submission_id}", body: body, headers: { "authorization" => @auth_token})
